...
const data: DataPoint[] = [
  { label: "Erro Tipo 3", value: 10 },
  { label: "Erro Tipo 2", value: 8 },
...
export default function Barra() {
  ...
  const dynamicBarHeight = Math.max(
    Math.min(availableSpace / dataPoints.length, MAX_BAR_HEIGHT),
    MIN_BAR_HEIGHT
  );
  ...
  const xDomain = dataPoints.map((dataPoint: DataPoint) => dataPoint.label);
  ...
  const pathElements = dataPoints.map((dataPoint: DataPoint, index: number) => {
